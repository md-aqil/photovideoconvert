...
abstract class BaseRepository
{
    ...
    function firstOrFailWhere($column, $value)
    {
    ...
    function uploadAttachment($model, $file, $request = '')
    {
        try {
            ...
            if ($url) {
                $model->attachments()->create([
                    ...
                    'url' => $url,
                    'relation_type' => AttachmentTypeEnum::DEFAULT->value,
                    'type' => $request->type ?? AttachmentTypeEnum::DEFAULT->value
                ]);
            }
        } catch (\Throwable $th) {
            Log::error($th);
